# This is the same as test_Szekeres.py with the functions S(r), P(r), Q(r)
# removed. The LTB limit of the Szekeres model is realized when 
# S(r), P(r), Q(r) constants.
...
i = 0
for tup in ans:
	Rdash_vec[i,:] = spR.ev(r_vector[i],t_vector,dx=1,dy=0)
	i = i + 1
spRdash = spline_2d(r_vector,t_vector,Rdash_vec,s=0)
i = 0
for tup in ans:
...
for r_val in r_vector:
	...
	print spRdash.ev(r_val,model_age)/np.sqrt(1.+2.*LTBw_E(r_val))*spR.ev(r_val,model_age)
def S(r):
	return 1e-5 #1.
def dS_dr(r):
	return 0.
...
theta = np.linspace(1e-5,2.*np.pi,13,endpoint=True)#12) when a=pi/2, b=pi then caos
phi = np.linspace(1e-5,2.*np.pi,13,endpoint=False)#12)
...
#init_dir = (4.5378560551852569, 5.291103416572283)
#init_dir = (0.69813170079773179, 5.6217973801080507)
#init_dir =(0.84106867056793033, 5.497787143782138)
init_dir = (0.52368210893163214, 0.0)
ans_central = model_geodesics(
              [model_age,r_vector[0]+loc*0., (90.+29.3)*np.pi/180.,276.4*np.pi/180.],#np.pi/2.,np.pi/4.],#
              (np.pi/2.,np.pi))#(3.2*np.pi/2.,0.37*np.pi))#(29.3*np.pi/180.,276.4*np.pi/180.)) (np.pi/2.+29.3*np.pi/180.,276.4*np.pi/180.)
...
theta_cent = np.arctan(tan_halfTheta)*2.
phi_cent   = np.arctan( (Q(r_cent)-q_cent)/(P(r_cent)-p_cent))
plt.figure()
plt.plot(geo_z_vec,p_cent)
plt.xscale('log')
plt.figure()
plt.plot(geo_z_vec,q_cent)
plt.xscale('log')
plt.figure()
plt.plot(geo_z_vec,theta_cent)
plt.xscale('log')
plt.figure()
plt.plot(geo_z_vec,phi_cent)
plt.xscale('log')
plt.figure()
plt.plot(geo_z_vec,1./(1.-np.cos( np.arctan(tan_halfTheta)*2.)))
plt.xscale('log')
plt.show()
import sys
#sys.exit()
def geo_loop(angle):
	return model_geodesics(
	       [model_age,r_vector[0]*0.+loc, (90.+29.3)*np.pi/180.,276.4*np.pi/180.],#np.pi/2.,np.pi/4.],#(90.)*np.pi/180.,276.4*np.pi/180.],
           angle)
...
for i in np.arange(num_angles):
	...
	###########3
	sp_t = spline_1d(geo_z_vec,geo_t[i,:],s=0)
	age_central = sp_center_t(z_ls)
	@Findroot
	def z_at_tdec(z,gamma):
		return sp_t(z)/age_central - 1.
	z_at_tdec.set_options(xtol=1e-8,rtol=1e-8)
	z_at_tdec.set_bounds(z_ls-300.,z_ls+300.) 
	#print "z_at_tdec.root ", z_at_tdec.root(0.), angles[i], i 
	###########
...
zdec_map = (z_ls-zdec_map)/(1.+zdec_map)*2.7255
