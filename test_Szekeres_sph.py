...
from Szekeres_sph import Szekeres_geodesics
...
model_geodesics = Szekeres_geodesics(spR,spRdash,spRdashdash,spRdashdot,spRdot,
                                     ...
                                     P,dP_dr,ddP_drr,
                                     Q,dQ_dr,ddQ_drr,
                                     S,dS_dr,ddS_drr,num_pt=3200)#1700
...
ans_central = model_geodesics(
              ...
              (-np.pi/2.,0.))
#theta, a, phi, b 1.57079632679 1e-05 4.82409005251 5.10508806
#ans_central = model_geodesics(
#              [model_age,r_vector[0]+loc*0., (90.)*np.pi/180.,276.4*np.pi/180.*0.],
...
def geo_loop(angle):
	return model_geodesics(
	       [model_age,r_vector[0]+loc, (90.+29.3)*np.pi/180.,276.4*np.pi/180.],
           angle)
...
for i in range(geo_t[:,0].size):
	...
	plt.yscale('log')
plt.figure()
for i in range(geo_r[:,0].size):
	plt.plot(geo_z_vec,geo_r[i,:])
	plt.xscale('log')
	plt.yscale('log')
plt.figure()
for i in range(geo_theta[:,0].size):
	plt.plot(geo_z_vec,geo_theta[i,:])#geo_theta[i,:])
	plt.xscale('log')
	#plt.yscale('log')
plt.figure()
for i in range(geo_phi[:,0].size):
	plt.plot(geo_z_vec,geo_phi[i,:])
	plt.xscale('log')
	#plt.yscale('log')
plt.show()
...
#                     ))
#plt.figure()
a, b = hp.pix2ang(64,np.arange(hp.nside2npix(64)))
